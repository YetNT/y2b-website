...
const commandsObject = {
    economy: [
        ...
        },
        {
            ...
            use: "/buy [item:{choices}] [amount:Number]",
        },
        {
            name: "promocode",
            description:
        ...
        },
        {
            ...
            use: "/crystalize [amount:Number (higher than 50)]",
        },
        {
            name: "forge",
            description: "Forge items from other items and list forgable items",
            use: "/forge",
        },
    ],
    other: [
        ...
        },
        {
            ...
            subcommand: [
                {
                    name: "list",
                    description: "list disabled and enabled commands",
                    use: "/command list",
                },
                {
                    name: "enable",
                    description:
                        "Enable a disabled command. Requires Manage Members Permission",
                    use: "/command enable [command:{choices}]",
                },
                {
                    name: "disable",
                    description:
                        "Disables an enabled command. Requires Manage Members Permission",
...
function cmd() {
    ...
    for (i in economyCommands) {
        if (economyCommands[i].subcommand === undefined) {
            ...
            for (x in economyCommands[i].subcommand) {
                subcommands.push(
                    `<subcommand id="${economyCommands[i].subcommand[x].name}"> <div class="subcommandHead"><div class="subcommandName">${economyCommands[i].subcommand[x].name}</div><div class="subcommandUse">${economyCommands[i].subcommand[x].use}</div></div><div class="subcommandDesc">${economyCommands[i].subcommand[x].description}</div></subcommand>`
                );
            }
    ...
    for (i in otherCommands) {
        if (otherCommands[i].subcommand === undefined) {
            outputAr.push(
                `<c class='command' id="${
                    otherCommands[i].name
                }"> <div class='commandHead'> <div class="commandName"> ${capFirstLetter(
                    otherCommands[i].name
                )} </div> <div class='commandUse'> ${
                    otherCommands[i].use
                } </div></div><div class='commandDesc'>${
                    otherCommands[i].description
                }</div></c>`
            );
        } else {
            let subcommands = [];
            ...
            for (x in otherCommands[i].subcommand) {
                subcommands.push(
                    `<subcommand id="${otherCommands[i].subcommand[x].name}"> <div class="subcommandHead"><div class="subcommandName">${otherCommands[i].subcommand[x].name}</div><div class="subcommandUse">${otherCommands[i].subcommand[x].use}</div></div><div class="subcommandDesc">${otherCommands[i].subcommand[x].description}</div></subcommand>`
                );
            }
            subcommands.push(`</div></div></c>`);
            let outputSub = subcommands.join("");
